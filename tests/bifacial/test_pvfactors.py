...
from xm_solarlib.bifacial.pvfactors import pvfactors_timeseries, PvfactorsTimeseriesParams
from ..conftest import assert_series_equal
import pytest

@pytest.fixture
def example_values():
    params = PvfactorsTimeseriesParams(
        timestamps=pd.DatetimeIndex([datetime(2017, 8, 31, 11), datetime(2017, 8, 31, 12)]),
        solar_zenith=[20., 10.],
...
def test_pvfactors_timeseries_pandas(example_values):
    ...
    inputs, outputs = example_values
    for key in ['solar_zenith', 'solar_azimuth', 'surface_tilt', 'surface_azimuth', 'dni', 'dhi']:
        setattr(inputs, key, pd.Series(getattr(inputs, key), index=inputs.timestamps))  # Use setattr and getattr
...
def test_pvfactors_scalar_orientation(example_values):
    ...
    # GH 1127, GH 1332
    inputs, outputs = example_values
    inputs.surface_tilt = 10.  # Use dot notation
    inputs.surface_azimuth = 90.  # Use dot notation
