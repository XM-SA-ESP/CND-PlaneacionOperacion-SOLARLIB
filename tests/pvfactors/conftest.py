...
from  xm_solarlib.pvfactors.geometry.base import ShadeCollection, PVSegment, PVSurface
from  xm_solarlib.pvfactors.geometry.pvrow import PVRowSide
from  xm_solarlib.pvfactors.geometry.pvarray import OrderedPVArray
import pytest
import os
import pandas as pd
import xm_solarlib
from xm_solarlib.constantes import TZ_US_ARIZONA
...
def generate_tucson_clrsky_met_data():
    """Helper function to generate timeseries data, taken from xm_solarlib
    documentation"""
    # Define site and timestamps
    latitude, longitude, tz, altitude = 32.2, -111,TZ_US_ARIZONA, 700
    times = pd.date_range(start='2019-01-01 01:00', end='2020-01-01',
                          freq='60Min', tz=tz)
    gcr = 0.3
    max_angle = 50
    # Calculate MET data
    solpos = xm_solarlib.solarposition.get_solarposition(times, latitude, longitude)
    apparent_zenith = solpos['apparent_zenith']
    azimuth = solpos['azimuth']
    airmass = xm_solarlib.atmosphere.get_relative_airmass(apparent_zenith)
    pressure = xm_solarlib.atmosphere.alt2pres(altitude)
    airmass = xm_solarlib.atmosphere.get_absolute_airmass(airmass, pressure)
    linke_turbidity = xm_solarlib.clearsky.lookup_linke_turbidity(times, latitude,
                                                            longitude)
    dni_extra = xm_solarlib.irradiance.get_extra_radiation(times)
    ineichen = xm_solarlib.clearsky.ineichen(
        apparent_zenith, airmass, linke_turbidity, altitude, dni_extra)
    # Calculate single axis tracking data
    trk = xm_solarlib.tracking.singleaxis(apparent_zenith, azimuth,
...
def pvmodule_canadian():
    """Example of xm_solarlib PV module name to use in tests"""
