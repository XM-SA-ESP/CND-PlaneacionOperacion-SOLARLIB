...
# 1. use module-level _projectName = 'xm_solarlib' and
#    _projectWarning = 'xm_solarlib_deprecation_warning' in place of MPL specific
#    string/Class.
...
class xm_solarlib_deprecation_warning(UserWarning):
    """A class for issuing deprecation warnings for xm_solarlib users.
...
_projectName = 'xm_solarlib'
_projectWarning = xm_solarlib_deprecation_warning
...
def _generate_deprecation_message(
    ...
    # Handling the 'removal' part
    if removal == "":
        removal = "soon"
    elif removal and pending:
        raise ValueError("A pending deprecation cannot have a scheduled removal")
    elif removal:
        removal = "in {}".format(removal)

    # Building the main deprecation message
    if not message:
        deprecation_message = "The %(name)s %(obj_type)s"
        if pending:
            deprecation_message += " will be deprecated in a future version"
        else:
            deprecation_message += " was deprecated in %(projectName)s %(since)s"
            if removal:
                deprecation_message += " and will be removed %(removal)s"
    
    # Adding alternative and addendum if present
    if alternative:
        deprecation_message += ". Use %(alternative)s instead."
    if addendum:
        deprecation_message += " %(addendum)s"

    # Formatting the final message
    return deprecation_message % dict(
