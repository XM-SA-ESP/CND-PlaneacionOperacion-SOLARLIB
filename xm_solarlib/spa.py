"Calcula la posición solar utilizando el algoritmo NREL SPA, ya sea utilizando matrices numpy o compilando el código a lenguaje de máquina con numba."
...
# Este bloque es una forma de utilizar una variable de entorno para alternar
# entre compilar las funciones con numba o simplemente usar numpy.
def nocompile(*args, **kwargs):
    return lambda func: func


if os.getenv('XM_SOLARLIB_USE_NUMBA', '0') != '0':
    try:
        ...
        if int(major + minor) >= 17:
            # Se necesita al menos numba >= 0.17.0
            jcompile = jit
            USE_NUMBA = True
        else:
...
# coheficientes de longitud heliocéntrica
L0 = np.array([
    [175347046.0, 0.0, 0.0],
...
# coeficientes de latitud heliocéntrica
B0 = np.array([
...
# coeficientes de radio heliocéntrico
R0 = np.array([
    [100013989.0, 0.0, 0.0],
...
# coeficientes de nutación de la longitud y oblicuidad
...
def julian_day_dt(year, month, day, hour, minute, second, microsecond):
    "Esta es la forma original de calcular el día juliano según el artículo de NREL. Sin embargo, es mucho más rápido convertirlo a tiempo Unix/época y luego a día juliano. Ten en cuenta que la fecha debe estar en UTC."
    if month <= 2:
        year = year-1
        month = month+12
    a = int(year/100)
...
# Omitir la firma de tipo aquí; especificar arrays de solo lectura requiere el uso de la API numba.types,
# lo que significa que numba debe estar disponible para importar.
# https://github.com/numba/numba/issues/4511
@jcompile(nopython=True)
def sum_mult_cos_add_mult(arr, x):
    # Cálculo compartido utilizado para la longitud, latitud y radio heliocéntricos
    s = 0.
...
def longitude_obliquity_nutation(julian_ephemeris_century, x0, x1, x2, x3, x4,
    ...
    # Parece que deberíamos poder devolver una tupla aquí en lugar de
    # recurrir a `salida`, pero devolver un UniTuple desde esta
    # función causaba que los cálculos en otros lugares dieran el resultado incorrecto.
    # Muy difícil de investigar ya que no ocurría cuando se usaba
    # el modo objeto. Se observó el problema en numba 0.56.4.
    out[0] = delta_psi
    out[1] = delta_eps
...
def local_hour_angle(apparent_sidereal_time, observer_longitude,
                     sun_right_ascension):
    "Medido hacia el oeste desde el sur"
...
def atmospheric_refraction_correction(local_pressure, local_temp,
                                      ...
                                      atmos_refract):
    # El interruptor establece delta_e cuando el sol está por debajo del horizonte
...
def solar_position_loop(unixtime, loc_args, out):
    "Recorre el arreglo de tiempo y calcula la posición solar"
    lat = loc_args[0]
    lon = loc_args[1]
    elev = loc_args[2]
    pressure = loc_args[3]
...
def solar_position_numba(unixtime, lat, lon, elev, pressure, temp, delta_t,
    ...
    "Calcula la posición solar utilizando las funciones compiladas con numba y múltiples hilos. Muy lento si las funciones no están compiladas con numba."
    # estos argumentos son iguales para cada hilo
    loc_args = np.array([lat, lon, elev, pressure, temp, delta_t,
                         atmos_refract, sst, esd])

    # construye un arreglo de dimensiones x ulength para almacenar los resultados
    ...
    # divide los arreglos de entrada y salida en numthreads trozos
...
def solar_position_numpy(unixtime, lat, lon, elev, pressure, temp, delta_t,
                         atmos_refract, numthreads, sst=False, esd=False):
    "Calcula la posición solar asumiendo que unixtime es un array de numpy. Ten en cuenta que esta función no funcionará si las funciones de posición solar fueron compiladas con numba."
...
def solar_position(unixtime, lat, lon, elev, pressure, temp, delta_t,
    ...
    """
    Calcula la posición solar utilizando el algoritmo NREL SPA descrito en [1].

    Si se tiene instalado numba, las funciones se pueden compilar y el código se ejecutará rápidamente. Si no, las funciones seguirán evaluándose pero utilizando numpy en su lugar.

    Parámetros
    ----------
    unixtime : arreglo de numpy
        Arreglo de marcas de tiempo unix/época para las cuales se calculará la posición solar. Unixtime es el número de segundos desde el 1 de enero de 1970 00:00:00 UTC. Un pandas.DatetimeIndex se puede convertir fácilmente usando .astype(np.int64)/10**9.
    lat : flotante
        Latitud para la cual se calculará la posición solar.
    lon : flotante
        Longitud para la cual se calculará la posición solar.
    elev : flotante
        Elevación del lugar en metros.
    pressure : entero o flotante
        Presión promedio anual en el lugar en milibares; se utiliza para la corrección atmosférica.
    temp : entero o flotante
        Temperatura promedio anual en el lugar en grados Celsius; se utiliza para la corrección atmosférica.
    delta_t : flotante
        Diferencia entre el tiempo terrestre y UT1.
    atmos_refrac : flotante
        La refracción atmosférica aproximada (en grados) en el amanecer y el atardecer.
    numthreads: entero, opcional, default 8
        Número de hilos a utilizar para el cálculo si numba>=0.17 está instalado.
    sst : booleano, default False
        Si es True, devuelve solo los datos necesarios para los cálculos de amanecer, atardecer y tránsito.
    esd : booleano, default False
        Si es True, devuelve solo la distancia Tierra-Sol en UA.

    Retorna
    -------
    Arreglo de numpy con elementos:
        cenit aparente,
        cenit,
        elevación,
        elevación aparente,
    ...
    Referencias
    ----------
    [1] I. Reda y A. Andreas, Algoritmo de posición solar para aplicaciones de radiación solar. Energía Solar, vol. 76, núm. 5, pp. 577-589, 2004.

    [2] I. Reda y A. Andreas, Corrección al algoritmo de posición solar para aplicaciones de radiación solar. Energía Solar, vol. 81, núm. 6, p. 838, 2007.
    """
    if USE_NUMBA:
...
def transit_sunrise_sunset(dates, lat, lon, delta_t, numthreads):
    """
    Calcula el tránsito solar, el amanecer y el atardecer
    para un conjunto de fechas en una ubicación dada.

    Parámetros
    ----------
    dates : arreglo
        Arreglo de numpy de enteros/flotantes correspondientes al tiempo Unix
        para las fechas de interés, debe ser la medianoche UTC (00:00+00:00)
        en el día de interés.
    lat : flotante
        Latitud de la ubicación para la cual se realizará el cálculo.
    lon : flotante
        Longitud de la ubicación.
    delta_t : flotante
        Diferencia entre el tiempo terrestre y UT. El USNO tiene tablas.
    numthreads : entero
        Número de hilos a utilizar para el cálculo (si se utiliza numba)

    Retorna
    -------
    tupla : (tránsito, amanecer, atardecer) localizados en UTC
    """
    ...
    # índice 0 es v, 1 es alpha, 2 es delta
    ...
    # se necesita tener en cuenta las fracciones del día que pueden ser el día siguiente o anterior en UTC
    add_a_day = m[2] >= 1
    sub_a_day = m[1] < 0
    m[1] = m[1] % 1
    m[2] = m[2] % 1
...
def calculate_deltat(year, month):
    """
    Calcula la diferencia entre el Tiempo Dinámico Terrestre (TD) y el Tiempo Universal (UT).

    Nota: Esta función aún no es compatible con cálculos que usen Numba.

    Las ecuaciones se toman de http://eclipse.gsfc.nasa.gov/SEcat5/deltatpoly.html
    """

    plw = 'Deltat es desconocido para años antes de -1999 y después de 3000. ' \
          'Los valores Delta se calcularán, pero los cálculos ' \
          'no están destinados a ser utilizados para estos años.'
